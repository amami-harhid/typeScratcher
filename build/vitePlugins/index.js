...
function transformLoopBody$1(node, visit, id) {
  ...
  if (ts.isBlock(node)) {
    ...
    for (const stmt of node.statements) {
      if (isTarget$1(stmt)) {
        newStatements2.push(createYieldStatement$1());
      }
...
function transformIfBody$1(node, visit) {
  if (ts.isBlock(node)) {
    ...
    if (isTarget$1(node)) {
      return ts.factory.createBlock([createYieldStatement$1(), ts.visitNode(node, visit)], true);
    }
...
const createTransformer$1 = (id, context) => {
  return (sf) => {
    ...
    transform(code, id) {
      ...
      try {
        const transpileResult = ts.transpileModule(code, {
          ...
          transformers: {
            before: [
              (context) => createTransformer$1(id, context)
            ]
          }
        });
...
const TsAstReplacer = TsAstReplacer$1;
const TsCodeReplacer = TsCodeReplacer$1;
