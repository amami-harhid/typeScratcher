...
debugモードで実行、クローンを多数作ると event.onが多すぎるエラー(11個以上)
Pen機能で使われる event.on のようだが、致命的バグの気配あり。

# 0.0.70

２つのスプライトでクローンをすると、両方とも同じクローンされたときのイベントが起動する（二重に起動する）
--> 解消済。


# 0.0.70(test/024)

costume.name = fontImageDummy 、ありえない名前になっている。
fontImageDummy は、文字列をSVG画像にする FontImageクラス( Imageクラスを継承 )にて付与するダミー名
異なるスプライトで使っているImageなのに、名前が混入してしまうのが不思議。致命的バグの気配がする
【原因】
触っているスプライトをGETしてTYPE=2,3のコスチューム名を判定しているが、「Hello World」の文字に触れている
とき、Cage以外のスプライトに触っていることになっていた。



# 0.0.70 

STOP(赤マーク)クリックを受けるイベント定義が欲しい。
